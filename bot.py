...
bot_users_col = db["bot_users"]  # everyone who has ever /start'd this bot (for /broadcast)
topics_col = db["topics"]  # per-student forum-topic mapping: {_id: user_id, thread_id, name, username, banned}
admins_col = db["admins"]  # additional admins granted the same access as OWNER_ID: {_id: user_id, added_at}


async def is_admin(user_id: int) -> bool:
    if user_id == OWNER_ID:
        return True
    doc = await asyncio.to_thread(admins_col.find_one, {"_id": user_id})
...
@app.on_message(filters.command("autocourse") & filters.private & admin_filter)
...
async def admin_autocourse_channel(client, message: Message):
    ...
    topics_structured = [
        {"name": topic, "video": counts["video"], "pdf": counts["pdf"], "count": counts["video"] + counts["pdf"], "children": []}
        for topic, counts in topic_counts.items()
    ]
    summary = f"{total_video} video lecture(s) and {total_pdf} PDF(s) across {len(topic_counts)} topic(s)."
    ...
    topic_lines = "\n".join(
        f"- {t['name']}: " + ", ".join(
            p for p in [f"{t['video']} video(s)" if t["video"] else "", f"{t['pdf']} PDF(s)" if t["pdf"] else ""] if p
        )
        for t in topics_structured
    )
    max_preview_chars = 3000
    if len(topic_lines) > max_preview_chars:
        truncated = topic_lines[:max_preview_chars]
        truncated = truncated[:truncated.rfind("\n")]
        topic_lines = truncated + f"\n… and more (see the full list in the store itself)."
...
# items per topic rather than trying to split video vs PDF. Any number of
# leading bracket groups is supported — (Topic), (Topic)(SubTopic),
# (Topic)(SubTopic)(SubSubTopic), and so on — building a nested structure
# to any depth.
# ---------------------------------------------------------------------------
SYLLABUS_LINE_RE = re.compile(r"^((?:\([^()]*\)\s*)+)(.*?)\s*:\s*(https?://\S+)\s*$")
SYLLABUS_BRACKET_RE = re.compile(r"\(([^()]*)\)")


def build_topic_tree(path_counts):
    """Turns {(A,): 3, (A, B): 5, (A, C): 2, ...} into a nested list:
    [{"name": "A", "count": 10, "children": [
        {"name": "B", "count": 5, "children": []},
        {"name": "C", "count": 2, "children": []},
    ]}]
    Works for any nesting depth, including a mix of depths under the same
    top-level topic."""
    root = {}

    def get_node(container, name):
        if name not in container:
            container[name] = {"name": name, "count": 0, "_children": {}}
    ...
    for path, count in path_counts.items():
        ...
        for level_name in path:
            node = get_node(container, level_name)
            node["count"] += count
...
@app.on_message(filters.command("importsyllabus") & filters.private & admin_filter)
...
async def admin_importsyllabus_thumbnail(client, message: Message):
    ...
    await message.reply_text(
        "Step 2: now send me the .txt file for this batch — lines like "
        "\"(Topic) Caption : URL\" or \"(Topic) (SubTopic) Caption : URL\" "
        "(any number of nested topic levels is fine). I'll count how many "
        "items each topic — and sub-topic — has."
    )
...
async def admin_importsyllabus_file(client, message: Message):
    ...
    path_counts = {}   # (topic,) or (topic, subtopic) or deeper -> count
    total_items = 0
    ...
    for raw in lines:
        ...
        if not m:
            continue
        brackets_part, _caption, _url = m.groups()
        topic_path = tuple(b.strip() for b in SYLLABUS_BRACKET_RE.findall(brackets_part) if b.strip())
        if not topic_path:
            continue

        if topic_path[0].lower() == "course thumbnail":
            continue  # thumbnail was already provided manually in step 1

        path_counts[topic_path] = path_counts.get(topic_path, 0) + 1
        total_items += 1
    ...
    topics_structured = build_topic_tree(path_counts)
    top_level_names = [t["name"] for t in topics_structured]
    summary = f"{total_items} lecture(s)/material(s) across {len(topics_structured)} topic(s)."

    pending = await get_state("syl_pending", {})
    pending["lecture_count"] = total_items
    pending["subjects"] = ", ".join(top_level_names)
    pending["topics"] = topics_structured
    pending["description"] = summary
    ...
    def render_lines(nodes, indent=0):
        out = []
        for n in nodes:
            out.append(f"{'  ' * indent}- {n['name']}: {n['count']} total")
            out.extend(render_lines(n["children"], indent + 1))
        return out

    all_lines = render_lines(topics_structured)
    topic_lines = "\n".join(all_lines)
    # Telegram messages are capped at 4096 characters — truncate the
    # preview if needed. The full breakdown is already saved to the
    # database either way, this only affects what's shown here.
    max_preview_chars = 3000
    if len(topic_lines) > max_preview_chars:
        truncated = topic_lines[:max_preview_chars]
        truncated = truncated[:truncated.rfind("\n")]
...
# ---------------------------------------------------------------------------
@app.on_message(filters.command("addcourse") & filters.private & admin_filter)
...
@app.on_message(filters.private & (filters.text | filters.photo) & admin_filter)
...
@app.on_message(filters.command("listcourses") & filters.private & admin_filter)
...
@app.on_message(filters.command("removecourse") & filters.private & admin_filter)
...
async def cmd_start(client, message: Message):
    ...
    if await is_admin(message.from_user.id):
        await message.reply_text(
            ...
            "<b>Support Desk</b>\n"
            "/setgroup — Link a group so students can chat with you directly\n\n"
            "<b>Visitors</b>\n"
            "/visitors — See who has opened the store, and how many times\n"
            "/exportvisitors — Download the full visitor list as a file\n"
            "/exportusers — Download everyone who has started this bot as a file\n\n"
            "<b>Broadcast</b>\n"
            "/broadcast — Send anything (text/photo/video/doc) to everyone who has ever started this bot\n\n"
            + (
                "<b>Admin Management</b> (owner only)\n"
                "/addadmin &lt;user_id&gt; — Let another user manage this bot\n"
                "/removeadmin &lt;user_id&gt; — Revoke a user's access\n"
                "/listadmins — Show everyone with access\n\n"
                if message.from_user.id == OWNER_ID else ""
            ) +
            "Send /help for a full explanation of how each command works.",
            parse_mode=ParseMode.HTML,
        )
    else:
        await message.reply_text("👋 Welcome! Tap the store button to browse available courses, or just send a message here — our team will get back to you.")


# ---------------------------------------------------------------------------
# Support Desk — students can message this bot directly and chat with you.
# Each student gets their own forum topic in a linked support group; your
# replies there are relayed straight back to them.
# ---------------------------------------------------------------------------
@app.on_message(filters.command("setgroup") & filters.private & admin_filter)
async def admin_setgroup(client, message: Message):
    await set_state("awaiting_group_link", True)
    await message.reply_text(
        "<b>Link Support Group</b>\n\n"
        "Forward any message from your support group here.\n\n"
        "Requirements: Topics must be enabled in that group's settings, "
        "and this bot must be an admin there with Manage Topics permission.",
        parse_mode=ParseMode.HTML,
    )
...
async def admin_group_link(client, message: Message):
    if not await get_state("awaiting_group_link"):
        raise ContinuePropagation
    chat = message.forward_from_chat
    if not chat or chat.type != ChatType.SUPERGROUP:
        await message.reply_text("That doesn't appear to be a group message. Please forward directly from the group.")
        return
    await set_state("support_group_id", chat.id)
    await set_state("awaiting_group_link", False)
    await message.reply_text(f"✅ Support group linked: <b>{chat.title}</b>", parse_mode=ParseMode.HTML)


async def get_or_create_topic(client: Client, user):
    doc = await asyncio.to_thread(topics_col.find_one, {"_id": user.id})
    if doc and doc.get("thread_id"):
        return doc["thread_id"], doc.get("banned", False)

    group_id = await get_state("support_group_id")
    if not group_id:
        return None, False

    title = f"{user.first_name or 'Student'} | {user.id}"[:128]
    try:
        ...
        if thread_id is None:
            raise RuntimeError(f"Could not determine new topic id: {result}")
    except Exception as e:
    ...
    await asyncio.to_thread(
        ...
        {"_id": user.id},
        {"$set": {
            "thread_id": thread_id, "name": user.first_name or "",
            "username": user.username or "", "banned": False,
        }},
        upsert=True,
    )
    ...
    header = (
        ...
        f"<i>Reply in this topic to message the student directly.</i>"
    )
...
async def mirror_to_topic(client: Client, thread_id, sender_label, text):
    group_id = await get_state("support_group_id")
    if not group_id or not thread_id:
        return
    try:
        await client.send_message(
            int(group_id), f"<b>{sender_label}:</b> {text}",
...
@app.on_message(
    filters.private & filters.text & ~admin_filter
    & ~filters.command(["start", "help"])
)
async def customer_message(client, message: Message):
    thread_id, banned = await get_or_create_topic(client, message.from_user)
    if banned:
        return
    await mirror_to_topic(client, thread_id, message.from_user.first_name or "Student", message.text)
...
async def group_topic_reply(client, message: Message):
    ...
    doc = await asyncio.to_thread(topics_col.find_one, {"thread_id": thread_id})
    if not doc:
        return
    ...
    try:
        ...
        logger.error(f"Failed to forward reply to student {user_id}: {e}")
        await message.reply_text(f"⚠️ Couldn't deliver to student: {e}")
...
async def ban_callback(client, query: CallbackQuery):
    ...
    await asyncio.to_thread(topics_col.update_one, {"_id": user_id}, {"$set": {"banned": True}})
    await query.answer("Student banned.", show_alert=True)
    try:
...
# ---------------------------------------------------------------------------
# Admin management — owner only, so admins can't grant further admins
# ---------------------------------------------------------------------------
...
async def owner_addadmin(client, message: Message):
    ...
    if new_admin_id == OWNER_ID:
        await message.reply_text("That's already you — the owner.")
        return
    await asyncio.to_thread(
    ...
    await message.reply_text(f"✅ <code>{new_admin_id}</code> can now manage this bot — full access, same as you.", parse_mode=ParseMode.HTML)
...
async def owner_removeadmin(client, message: Message):
    ...
    result = await asyncio.to_thread(admins_col.delete_one, {"_id": removed_id})
    await message.reply_text("✅ Removed." if result.deleted_count else "❌ That user wasn't an admin.")
...
async def owner_listadmins(client, message: Message):
    admins = await asyncio.to_thread(lambda: list(admins_col.find()))
    lines = [f"👑 <code>{OWNER_ID}</code> — owner"]
    lines += [f"• <code>{a['_id']}</code>" for a in admins]
    await message.reply_text("<b>Bot Admins</b>\n\n" + "\n".join(lines), parse_mode=ParseMode.HTML)


@app.on_message(filters.command("broadcast") & filters.private & admin_filter)
async def admin_broadcast_start(client, message: Message):
    await set_state("broadcast_step", "content")
    await message.reply_text(
        "<b>Broadcast</b>\n\n"
        "Send me the message you want to broadcast — text, a photo, a video, "
        "or a document, with or without a caption. Whatever you send is "
        "exactly what everyone will receive.",
        parse_mode=ParseMode.HTML,
    )


@app.on_message(filters.private & admin_filter)
async def admin_broadcast_content(client, message: Message):
    if await get_state("broadcast_step") != "content":
        raise ContinuePropagation
    if message.text and message.text.startswith("/"):
        raise ContinuePropagation

    await set_state("broadcast_step", None)

    all_users = await asyncio.to_thread(lambda: list(bot_users_col.find()))
    if not all_users:
        await message.reply_text("No one has started this bot yet.")
        return

    status_msg = await message.reply_text(f"📤 Sending... 0/{len(all_users)}")
    sent, failed = 0, 0
    for u in all_users:
        # Prefer @username when we have one — Telegram can resolve a
        # username fresh even if this bot's local peer cache was wiped
        # (e.g. by a Heroku restart), whereas a bare numeric ID can only
        # be resolved from cache, causing "Peer id invalid" failures.
        target = f"@{u['username']}" if u.get("username") else u["_id"]
        try:
            await message.copy(target, protect_content=True)
            sent += 1
        ...
        if (sent + failed) % 20 == 0:
            try:
                await status_msg.edit_text(f"📤 Sending... {sent + failed}/{len(all_users)}")
            except Exception:
                pass

    await status_msg.edit_text(
        f"✅ Broadcast complete.\nSent: {sent}\nFailed: {failed}\n\n"
        f"{'Note: failures are usually people without a public @username whose chat this bot lost track of after a restart — nothing to fix on your end, they need to message the bot once more.' if failed else ''}"
    )


@app.on_message(filters.command("help") & filters.private & admin_filter)
async def cmd_help(client, message: Message):
    await message.reply_text(
        ...
        "<b>Broadcast</b>\n"
        "/broadcast then send whatever you want (text, photo, video, or "
        "document) — it goes to everyone who has ever sent /start to this "
        "bot (a separate list from store Visitors, since it doesn't "
        "require opening the Mini App).\n\n"

        "<b>Support Desk</b>\n"
        "/setgroup links a Topics-enabled group as your support desk. "
        "Once linked, any student who messages this bot gets their own "
        "topic in that group automatically — reply there and it goes "
        "straight back to them. A \"Ban\" button on each new conversation "
        "lets you stop hearing from someone if needed.\n\n"
...
@app.on_message(filters.command("visitors") & filters.private & admin_filter)
async def admin_visitors(client, message: Message):
    ...
    await message.reply_text(
        ...
        f"Use /exportvisitors for the full list as a downloadable file.",
        parse_mode=ParseMode.HTML,
    )


@app.on_message(filters.command("exportvisitors") & filters.private & admin_filter)
async def admin_exportvisitors(client, message: Message):
    all_visitors = await asyncio.to_thread(lambda: list(visitors_col.find().sort("last_visit", -1)))
    if not all_visitors:
        await message.reply_text("No one has opened the store yet.")
        return

    path = "/tmp/visitors_export.csv"
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["user_id", "first_name", "username", "visit_count", "first_visit", "last_visit"])
        for v in all_visitors:
            writer.writerow([
                ...
                v.get("username", ""),
                v.get("visit_count", 1),
                time.strftime("%Y-%m-%d %H:%M", time.localtime(v["first_visit"])) if v.get("first_visit") else "",
                time.strftime("%Y-%m-%d %H:%M", time.localtime(v["last_visit"])) if v.get("last_visit") else "",
            ])

    await client.send_document(message.chat.id, path, caption=f"📄 {len(all_visitors)} visitor(s)")
    os.remove(path)


@app.on_message(filters.command("exportusers") & filters.private & admin_filter)
async def admin_exportusers(client, message: Message):
    all_users = await asyncio.to_thread(lambda: list(bot_users_col.find()))
    if not all_users:
        await message.reply_text("No one has started this bot yet.")
        return

    path = "/tmp/bot_users_export.csv"
    with open(path, "w", newline="", encoding="utf-8") as f:
        writer = csv.writer(f)
        writer.writerow(["user_id", "first_name", "username", "first_seen"])
        for u in all_users:
            writer.writerow([
                u["_id"],
                u.get("first_name", ""),
                u.get("username", ""),
                time.strftime("%Y-%m-%d %H:%M", time.localtime(u["first_seen"])) if u.get("first_seen") else "",
            ])

    await client.send_document(message.chat.id, path, caption=f"📄 {len(all_users)} user(s) who have started this bot")
    os.remove(path)
...
async def main():
    ...
    try:
        group_id = await get_state("support_group_id")
        if group_id:
            await app.get_chat(int(group_id))
    except Exception as e:
        logger.warning(f"Could not pre-warm peer cache for support group: {e}")
