...
async def admin_wizard_step(client, message: Message):
    ...
    if step_index is None or message.text.startswith("/"):
        return
    ...
    set_state("course_pending", {})
    await message.reply_text(f"✅ Course added (ID: <code>{course_id}</code>). It's now live in the store.", parse_mode="html")
...
def build_web_app():
    ...
    web_app.router.add_static("/", path=os.path.join(os.path.dirname(__file__), "webapp"), show_index=True)
    return web_app
