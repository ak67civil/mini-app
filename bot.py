...
async def admin_autocourse_channel(client, message: Message):
    ...
    topics_structured = [
        {"name": topic, "video": counts["video"], "pdf": counts["pdf"], "count": counts["video"] + counts["pdf"]}
...
async def admin_importsyllabus_file(client, message: Message):
    ...
    topics_structured = [{"name": topic, "video": 0, "pdf": 0, "count": count} for topic, count in topic_counts.items()]
    summary = f"{total_items} lecture(s)/material(s) across {len(topic_counts)} topic(s)."
    ...
    topic_lines = "\n".join(f"- {t['name']}: {t['count']} total" for t in topics_structured)
